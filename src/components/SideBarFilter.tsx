...
const SidebarFilters = () => {
  ...
  const [selectedCollection, setSelectedCollection] = useState("Men"); // Default selected value

  const handleCollectionSelect = (collectionName: string) => {
    setSelectedCollection(collectionName); // Update the selected collection
  };
  ...
  const renderCollections = () => {
    return (
      <div className="relative flex flex-col space-y-4 pb-8">
        <h3 className="mb-2.5 font-medium">Collections</h3>
        <CollectionSelect
          selected={selectedCollection} // Pass the selected collection
          onCollectionSelect={handleCollectionSelect} // Pass the selection handler
        />
      </div>
    );
  };
